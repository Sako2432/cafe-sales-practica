...
Aplicar técnicas de limpieza y transformación de datos con Python sobre `dirty_cafe_sales.csv`, el cual tiene varios errores en sus datos.
...
2. **Estandarización de faltantes**: `"ERROR"` y `"UNKNOWN"` cambiarán por `NaN` en todas las columnas, para poder tratarlos de la misma forma a todos.
3. **Corrección de tipos de datos**: `Quantity`, `Price Per Unit` y `Total Spent` se convierten a valor numérico; `Transaction Date` se convierte a formato de fecha.
4. **Reconstrucción de valores faltantes usando relaciones reales de los datos** (en vez de imputar con la media/moda de una manera como ciega):
...
6. **Categoría "Desconocido"** para lo que no se pudo reconstruir ni tenía sentido imputar: `Item` en casos de precio ambiguo (495 filas), y `Payment Method` / `Location`, que tenían alrededor 32 % y 40 % de datos faltantes respectivamente — imputar con la moda habría introducido un sesgo algo fuerte, así que se prefirió declarar el dato como desconocido para no inventarlo.
...
8. **Duplicados y valores atípicos**: se revisaron  y no se encontraron (no había filas duplicadas ni valores de `Quantity`/`Price Per Unit` fuera de un rango ), por lo que se dejaron asi.
...
```bash #no sabia que esto se ponia 
...
Se utilizó claude para dudas sobre como usar ipnyb y asistencia de debbug de codigo
